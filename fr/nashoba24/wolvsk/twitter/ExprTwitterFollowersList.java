...
public class ExprTwitterFollowersList extends SimpleExpression<User>{
	...
	protected User[] get(Event e) {
		...
		try {
			if(self) {
				ResponseList<User> list = WolvSKTwitter.tf.getInstance().getFollowersList(WolvSKTwitter.tf.getInstance().getAccountSettings().getScreenName(), -1);
				...
				ResponseList<User> list = WolvSKTwitter.tf.getInstance().getFollowersList(user.getSingle(e).getId(), -1);
				ArrayList<User> users = new ArrayList<User>();
				for(User u : list) {
					users.add(u);
				}
